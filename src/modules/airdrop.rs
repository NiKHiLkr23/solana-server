...
async fn request_airdrop(
    ...
    let pubkey = payload.public_key.parse::<Pubkey>()
    ...
    let signature = client.request_airdrop(&pubkey, amount_lamports)
        .map_err(SolanaError::ClientError)?;
    
    // Wait for confirmation
    let confirmation = client.confirm_transaction(&signature)
        .map_err(SolanaError::ClientError)?;
    ...
    Ok(Json(AirdropResponse {
        ...
        amount_lamports,
        message: format!("Successfully airdropped {} SOL to account", payload.amount_sol),
    }))
} 