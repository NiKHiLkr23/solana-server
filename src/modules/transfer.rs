...
async fn transfer_sol(
    ...
    let to_pubkey = payload.to_public_key.parse::<Pubkey>()
        .map_err(|_| SolanaError::InvalidInput("Invalid recipient public key format".to_string()))?;
    
    // Decode private key
    let private_key_bytes = base64::decode(&payload.from_private_key)
